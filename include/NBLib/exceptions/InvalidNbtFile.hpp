...
{
	class InvalidNbtFile : public std::exception
	{
	private:
	...
	public:
		/// \brief Initializes a new InvalidNbtFile error to be thrown.
		///
		/// \param[in] value The error message.
		/// \throws InvalidNbtFile error
		explicit InvalidNbtFile(std::string value) : message(std::string("[InvalidNbtFile] ") + std::move(value)) {}
		explicit InvalidNbtFile(const char *value) : message(std::string("[InvalidNbtFile] ") + value) {}
